...
#Inutile car la charge ne dispose pas de TC dans l'énoncé.
# path_folder = r'data'
...
path_file_solar = path_folder + "\Plant_2_Generation_Data.csv"
df_solar_data = pd.read_csv(path_file_solar,parse_dates=['DATE_TIME']).set_index('DATE_TIME')
df_solar_data = df_solar_data[df_solar_data['SOURCE_KEY']==df_solar_data['SOURCE_KEY'][0]] #Selection of one string
df_solar_data=df_solar_data.asfreq('5T',method='ffill')

df_solar_data['ppoc_measured']=np.random.randint(50,800), size=len(df_solar_data))
